...
/*   Updated: 2019/07/15 16:35:31 by sbecker          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	tool_pipette(t_guimp *g, t_ui_el *el)
{
	...
	color = ui_util_get_pixel_color_from_el(el->sdl_renderer,
			el, (t_vec2){el->ptr_rel_pos.x, el->ptr_rel_pos.y});
	...
	ui_el_set_new_pos(el, 0, PIXEL, (t_fvec2){-el->rect.w /
			2 + g->draw_tool.r / 255.0f * el->parent->rect.w, 0});
	el = ui_win_find_el_by_id(g->tool_win, GM_TOOL_ID_SL_HEAD_GR);
	ui_el_set_new_pos(el, 0, PIXEL, (t_fvec2){-el->rect.w /
			2 + g->draw_tool.g / 255.0f * el->parent->rect.w, 0});
	el = ui_win_find_el_by_id(g->tool_win, GM_TOOL_ID_SL_HEAD_BL);
	ui_el_set_new_pos(el, 0, PIXEL, (t_fvec2){-el->rect.w /
			2 + g->draw_tool.b / 255.0f * el->parent->rect.w, 0});
