...
/*   Updated: 2019/07/14 16:22:38 by sbecker          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void create_ok_cancel_button(t_ui_el *p, t_ui_text *ui_text, t_ui_win *w)
{
	...
	ui_el_set_text(el_text, ui_text->font, (t_text_params){(SDL_Color){0, 0, 0, 0},
                        (SDL_Color){0, 0, 0, 0}, 0, 0, TEXT_IS_BLENDED});
	ui_el_update_text(el_text, "OK");
	ui_el_setup_default(el_text);
	el_text->params |= EL_IGNOR_RAYCAST;
}

static void fill_modal_ok_cancel(t_ui_text *ui_text, t_ui_el *p, char **text, t_ui_win *w)
{
	t_ui_el *el;
	int     i;
	float   h;
